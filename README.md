...
下载 `MemeHelper-2.3.9-windows-x64.7z`，完整解压后运行其中的 `MemeHelper.exe`。应用已迁移到 Tauri，不再附带 Electron/Chromium 文件。
...
输出位于 `release-tauri/MemeHelper-2.3.9/`。同版本发布目录或压缩包已存在时，构建会停止并要求先更新版本号，不会覆盖正在运行或已经交付的旧版。构建机需要 Rust；Windows SDK 可由 Visual Studio Build Tools 提供，项目构建脚本也会自动使用本机已有的 `cargo-xwin` SDK 缓存。
